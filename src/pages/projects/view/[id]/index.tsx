...
import { deleteClientById } from 'apiSdk/clients';
import { deleteCostEstimateById } from 'apiSdk/cost-estimates';
...
function ProjectViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProjectInterface>(
    ...
    () =>
      getProjectById(id, {
        relations: ['organization', 'user', 'client', 'cost_estimate'],
      }),
  );
  ...
  const cost_estimateHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('client', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/clients/create?project_id=${data?.id}`}>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.client?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/clients/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>{record.email}</Td>
              ...
              {hasAccess('cost_estimate', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Cost Estimates
                    </Text>
                    <NextLink passHref href={`/cost-estimates/create?project_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.cost_estimate?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/cost-estimates/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('cost_estimate', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/cost-estimates/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('cost_estimate', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    cost_estimateHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
